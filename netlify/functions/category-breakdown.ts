...
import { CATEGORY_BREAKDOWN_SQL } from "./shared/queries";
...
export const handler: Handler = async (event) => {
  try {
    ...
    const categories = await executeQuery(CATEGORY_BREAKDOWN_SQL, [startDate, endDate]);
    ...
    console.error("Error in category-breakdown function:", error);
    return {
      statusCode: 500,
      headers,
      body: JSON.stringify({
